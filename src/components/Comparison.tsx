...
const features = {
  ...
  funders: {
    ...
    desc: "Support the mission to build privacy-first tools. Gain early-bird perks, roadmap influence, and 4 months premium free at full app launch.",
...
// ✅ Helper — dynamic API base (works for local + Vercel)
const BASE_URL =
  import.meta.env.VITE_API_BASE_URL || "http://localhost:5000"; // fallback for local
...
const WaitlistForm: React.FC<{ onDone: (ok: boolean, msg?: string) => void }> = ({ onDone }) => {
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok && data.success) onDone(true, data.message || "You joined the waitlist!");
...
const FunderForm: React.FC<{ onDone: (ok: boolean, msg?: string) => void }> = ({ onDone }) => {
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) onDone(true, data.message || "Funder interest recorded!");
      else onDone(false, data.error || "Server error");
    } catch {
      onDone(false, "Network error");
    } finally {
  ...
  return (
    <section className="w-full py-16 px-6 md:px-16 lg:px-24 bg-joyxora-dark text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
        {/* Waitlist */}
        <motion.div
          className="p-8 bg-joyxora-gray rounded-2xl flex flex-col justify-between"
        ...
        >
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-joyxora-green text-black font-bold"
          >
